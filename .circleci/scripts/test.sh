...
export CIRCLE_PROJECT_USERNAME=alvin-huang
export CIRCLE_PROJECT_REPONAME=consul
export CIRCLE_SHA1=5baa7b1b04a8b3b82359586efa0c57b0e0990647
...
function cherry_pick_with_slack_notification {
    ...
    if ! git cherry-pick --mainline 1 "$commit"; then
        ...
        echo "send fail slack notification"
        ...
        echo "send fail github pr comment"
    ...
    # Else we send a success notification
        ...
        echo "push changes and success slack"
        ...
        # post PR comment to GitHub
        echo "success github message"
        github_message=":cherries::white_check_mark: Cherry pick of commit ${commit} onto \`$branch\` succeeded!"
        pr_id=$(basename ${pr_url})
        curl -f -s -H "Authorization: token ${GITHUB_TOKEN}" \
             -X POST \
             -d "{ \"body\": \"${github_message}\"}" \
             "https://api.github.com/repos/${CIRCLE_PROJECT_USERNAME}/${CIRCLE_PROJECT_REPONAME}/issues/1/comments"
    fi
...
resp=$(curl -f -s -H "Authorization: token ${GITHUB_TOKEN}" "https://api.github.com/search/issues?q=repo:hashicorp/consul+sha:${CIRCLE_SHA1}")
ret="$?"
if [[ "$ret" -ne 0 ]]; then
...
for label in $labels; do
    ...
    if [[ $label =~ docs-cherrypick ]]; then
        ...
        (( backport_failures += "$?" ))
        echo "backport failures is: $backport_failures"
